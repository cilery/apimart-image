#!/usr/bin/env python3
"""Generate images with APIMart image models."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate images via APIMart.")
...
def run_single_task(client: ApimartClient, args: argparse.Namespace, task: dict, index: int = 0) -> list[Path]:
    ...
    validate_model_inputs(model_name, image_count=0, has_mask=False)
    options = task_to_options(task, args)
    ...
    status_payload = client.poll_task(
        task_id,
        poll_interval=args.poll_interval,
        timeout_seconds=args.timeout,
    )
    image_urls = extract_result_image_urls(status_payload)
    if not image_urls:
    ...
    for image_index, image_url in enumerate(image_urls):
        ...
        output_path = choose_output_path(
            output=output_override if len(image_urls) == 1 else None,
